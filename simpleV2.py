...
display = Display(resolution = (640, 480))
...
#    bin = cv2.bilateralFilter(bin, 30, 80, 120)
    bin =cv2.GaussianBlur(bin, (7, 7),0)
...
#    cv2.imshow("circles", circles)
    ...
    circles = cv2.HoughCircles(circles, cv2.cv.CV_HOUGH_GRADIENT,1.5 , 40 )
...
#                print("alt = {0}".format(con.shape[1]))
                for sec in xrange(0,parts-6):
                    pixel = ((r-ofsetX)+math.sin(2*math.pi/parts*sec)*(r*0.80), (r-ofsetY )+math.cos(2*math.pi/parts*sec)*(r*0.80))
...
#                        print(" cor = %r " % (con[pixel]))
            ...
            #                    con.drawText(txt)
                            ...
                            con[pixel]=(0,0,255)
                            final_img = cv2.resize(img_circs , (0,0), fx=0.7, fy=0.7)

                            cv2.imshow("circ", final_img)
